    ...
    return struct {
        ...
        pub fn write(self: *Self, data: []const u8, msg_type_id: i32) bool {
            ...
            while (true) {
                ...
                if (self.head.cmpxchgWeak(current_head, new_head, .acquire, .monotonic)) |_| {
                    // CAS failed, retry.
                    continue;
                }
                ...
                // Write the header with frame_length = 0 (uncommitted).
        ...
        pub fn read(self: *Self) ?ReadResult {
            ...
            self.tail = current_tail + total_len;
            @atomicStore(usize, &self.tail, self.tail, .release);
